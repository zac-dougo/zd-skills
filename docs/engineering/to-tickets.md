...
| The whole change fits in one context window | build it directly, skip the tickets |
...
Tickets that `to-tickets` produced are agent-ready by construction: each one carries its own brief and acceptance criteria.

## Prerequisites

`to-tickets` publishes into a tracker, so the repo must have one configured first, along with its label vocabulary. Either kind works: a real tracker like GitHub or Linear, or local markdown files under `.scratch/`, which is supported out of the box.
...
Upstream is [to-spec](https://aihero.dev/skills-to-spec), which hands it a settled spec to slice against; keep both in one unbroken context window. Downstream is the build itself: work one ticket per fresh session and close with [code-review](https://aihero.dev/skills-code-review).
