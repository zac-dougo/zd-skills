...
| Decided, and the work fits one [context window](https://www.aihero.dev/ai-coding-dictionary/context-window) | build it directly: skip the spec |
| Decided, and the work spans several sessions | `/to-spec`, then [to-tickets](https://aihero.dev/skills-to-tickets) |

## Prerequisites

`to-spec` publishes the spec as an issue, so the repo must have a tracker configured first, along with its label vocabulary. Either kind works: a real tracker like GitHub, or local markdown files under `.scratch/`, which is supported out of the box.
...
Its neighbour upstream is [grill-with-docs](https://aihero.dev/skills-grill-with-docs), which does the deciding this skill only records. Downstream, [to-tickets](https://aihero.dev/skills-to-tickets) cuts the spec into tracer-bullet tickets to build.
