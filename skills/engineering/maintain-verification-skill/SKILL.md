...
description: Audit a project's verification skill and feature map, shipping at most one PR of proven corrections.
...
0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map (usually `verify-*/` under the target repo's project skill directory). Several candidates → ask which one; none → stop and point at `create-verification-skill` instead of inventing a target.
