---
name: why
description: Investigate why code looks the way it does, with cited evidence and calibrated confidence.
...
Before spawning investigators, list the integrations available in your harness: integration servers, plugins, authenticated CLIs, and plain git/`gh`, which is always available. Use the harness's tool listing when present.

Map each available integration to one evidence category:
...
Source control is always available through git and `gh`. For the other six, classify using the integration name, server instructions, tool names, and resource descriptors. If an integration could fit more than one category, choose the one matching its primary evidence. Record ambiguous cases in the coverage map.

Aim for a complete **coverage map**, not a minimal one. Document the null, don't skip the search.

Launch all matching investigators in a single message so they run concurrently, or run them sequentially inline if your harness has no subagents. Don't ask one agent to cover multiple integrations.

Each investigator runs with full tool access: a restricted or ask mode would strip integration access, which disables integration-backed investigators entirely. Investigators still shouldn't write anything. Use a strong model your harness offers.
...
2. **Issue / ticket tracker investigator** (e.g. Linear, Jira, GitHub Issues, Plane, Shortcut). Best at surfacing *the product or business forcing function*. Strongest when the why is external to engineering.
...
- **No integration is available for that category** in this environment. Flag this as a gap, not a choice. Example: "Real-time team chat skipped. No matching integration available, so the conversational record was not searchable."
...
Spawn one synthesizer subagent with full tool access, using a strong model your harness offers, or synthesize inline if your harness has no subagents. The synthesizer's quality check spot-verifies citations, which can require integration access.
